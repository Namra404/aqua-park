    ...
    return (
        <div>
            <div className={'max-w-[250px] relative mb-4'}>
                <SlideCard {...slide}/>
                <Text className={'absolute top-2 left-2 bg-black py-0.5 px-2 rounded-xl'} white>{total_price}</Text>
                <ServiceModal
                    trigger={<Text className={'bg-[#ad78f8] rounded-bl-full absolute top-0 right-0 p-2'} white type={'small-bold'} asLink>+{services?.length}</Text>}
                >
